...
// template<size_t degree, size_t modulus, class T>
// int testPerfPSIBloom(uint64_t size,uint64_t reps)
// {
//
//     auto start = std::chrono::steady_clock::now();
//     auto end = std::chrono::steady_clock::now();
...
//
//  std::cout << "======================================================================" << std::endl;
//  std::cout << "PSI : n=" << size << " in " << (t1+t2+t3)/reps << " us " << std::endl;
//  std::cout << "PSI : initC " << size << " in " << (t1)/reps << " us " << (100*t1/(t1+t2+t3)) <<"%"<< std::endl;
//  std::cout << "PSI : initS " << size << " in " << (t2)/reps << " us " << (100*t2/(t1+t2+t3)) <<"%"<< std::endl;
//  std::cout << "PSI : finaC " << size << " in " << (t3)/reps << " us " << (100*t3/(t1+t2+t3)) <<"%"<< std::endl;
//
...
int run()
{
    ...
    // testencode_decode<degree,modulus,T>();
    // testencrypt_decrypt<degree,modulus,T>();
    // testPSI1<degree,modulus,T>(1024);
    // testPSI0<degree,modulus,T>(1024);
    // testPSIAND<degree,modulus,T>(1024);
    // for(int i=512;i<2*512;i+=1)
    //   testPSIAsLib<degree,modulus,T>(i);
    // testPSIAsLib<degree,modulus,T>(2*512);
    // testPSIAsLib<degree,modulus,T>(10*512);
    // testPSIAsLib<degree,modulus,T>(100*512);
    // testPSIAsLib<degree,modulus,T>(1000*512);
    // testPSIAsLib<degree,modulus,T>(2000*512);
    // testPSIAsLib<degree,modulus,T>(2*2000*512);
    //testPSIAsLib<degree,modulus,T>(3*2000*512);
    //testPSIAsLib<degree,modulus,T>(512*512);
    //testSymetricPSIAsLib<degree,modulus,T>(2000*512);
    testPerfPSI<degree,modulus,T>(100000,10);
    testPerfPSI<degree,modulus,T>(10000,10);
    testPerfPSI<degree,modulus,T>(1000,100);
    testPerfPSI<degree,modulus,T>(100,100);
    //test3PartiesPSIAsLib<degree,modulus,T>(512*512);

    // testHash<degree,modulus,T>(512,512);
    // testHash<degree,modulus,T>(1024,512);
    // testHash<degree,modulus,T>(2048,512);
    // testHash<degree,modulus,T>(4096,512);
