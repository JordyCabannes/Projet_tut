...
int main() {
    ...
    /*string filename="/vagrant/dev/krog/genData/laas.csv";
    string path="/vagrant/dev/krog/genData/_laas.csv";
    ...
    /*for(boost::tie(vi_heuristik, vend_heuristik) = vertices(g_heuristik); vi_heuristik != vend_heuristik; ++vi_heuristik)
        std::cout << " node : " << *vi_heuristik << std::endl;*/
    ...
    for (boost::tie(u_iter_heuristik, u_end_heuristik) = vertices(g_heuristik); u_iter_heuristik != u_end_heuristik; ++u_iter_heuristik){
        for (boost::tie(ei_heuristik, e_end_heuristik) = out_edges(*u_iter_heuristik, g_heuristik); ei_heuristik != e_end_heuristik; ++ei_heuristik){
            if ( (capacity_heuristik[*ei_heuristik]-residual_capacity_heuristik[*ei_heuristik] ) > 0 && weight_heuristik[*ei_heuristik] > 0 ){
                ...
                Driver _driver=carpooling::SampleGraph::findByPk_driver(*u_iter_heuristik-1,users);
                Privacy::Driver *d1=new Privacy::Driver(_driver.origin, _driver.destination);
                Driver _pedestrian= carpooling::SampleGraph::findByPk_passenger(target(*ei_heuristik, g_heuristik)-users.drivers.size()-1,users);
...
//-----------------ideal algorithm------------------
    ...
    /*for(boost::tie(vi_ideal, vend_ideal) = vertices(g_ideal); vi_ideal != vend_ideal; ++vi_ideal)
        std::cout << " node : " << *vi_ideal << std::endl;*/
    ...
    for (boost::tie(u_iter_ideal, u_end_ideal) = vertices(g_ideal); u_iter_ideal != u_end_ideal; ++u_iter_ideal){
        for (boost::tie(ei_ideal, e_end_ideal) = out_edges(*u_iter_ideal, g_ideal); ei_ideal != e_end_ideal; ++ei_ideal){
            if ( (capacity_ideal[*ei_ideal]-residual_capacity_ideal[*ei_ideal] ) > 0 && weight_ideal[*ei_ideal] > 0 ){
                ...
                Driver _driver1=carpooling::SampleGraph::findByPk_driver(*u_iter_ideal-1,users);
                Privacy::Driver *d2=new Privacy::Driver(_driver1.origin, _driver1.destination);
                Driver _pedestrian1= carpooling::SampleGraph::findByPk_passenger(target(*ei_ideal, g_ideal)-users.drivers.size()-1,users);
                ...
                //std::cout << "node " << *u_iter_ideal << " with node " << target(*ei_ideal, g_ideal) << std::endl;
                compt_ideal ++;
