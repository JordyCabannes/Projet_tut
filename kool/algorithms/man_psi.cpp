...
int matching() {
    ...
    // sampling a day in 5 minutes epoch
    //uint64_t size = 24*12;
    // initialization of users profiles

    uint64_t size = 24*60*60;
    uint8_t aliceProfile[size], bobProfile[size];
    auto start = std::chrono::steady_clock::now();
    auto end = std::chrono::steady_clock::now();

    // construction of users profiles (by default we fill everything )
    ...
    nfl::poly_from_modulus<T, degree, modulus> *SecretKey;
    char *aliceOut;
    uint64_t aliceOutLen;
    initiateC<degree,modulus,T>(aliceOut,(char*)aliceProfile,aliceOutLen,size,SecretKey);

    char *bobOut;
    uint64_t bobOutLen;
    initiateS<degree,modulus,T>(bobOut,aliceOut,(char*)bobProfile,bobOutLen,size);
    char match[size];
    terminateC<degree,modulus,T>(match,bobOut,bobOutLen,size,SecretKey);
    for(int i=0;i<size;i++){
        if(match[i]==1){
    ...
    std::cout << 1000*boost::lexical_cast<double>(get_time_us(start, end, 1)/1000000) << std::endl;
}
...
int launch() {
    ...
    string filename="/vagrant/ubimob/analysis/experiments/dec2015/groupII.csv";
    ...
    tb->create_result("/vagrant/ubimob/analysis/experiments/dec2015/Results/final/groupII.csv");
...
//###############################################################################################################
        ...
        if (m->shared_path_len() > 0){
            ...
            output.rider_pickup_size = tb->dlen2(r1->data_before);
            //size of potential pickup for driver
            output.driver_pickup_size = tb->dlen2(d1->data_before);
            //size of potential dropoff for pedesdrian
            output.rider_dropoff_size = tb->dlen2(r1->data_after);
            //size of potential dropoff for driver
            output.driver_dropoff_size = tb->dlen2(d1->data_after);
            ...
            tb->save_result("/vagrant/ubimob/analysis/experiments/dec2015/Results/final/groupII.csv", output, g);
