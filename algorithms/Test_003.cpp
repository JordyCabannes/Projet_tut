...
void save_position2(string fileName, Driver car, Pedestrian foot, Manager m, const Transport::Graph * g ){
    ...
    fout.close();}
void save_runtime(time_t isochrone_time, time_t psi_time, time_t matching_time, int scenario_id){
    
    string filename="/home/matchi/Desktop/ST/results007/Runtime"+std::to_string(scenario_id) +".csv";
    ofstream fout(filename);
    fout << "isochrone" <<','<< "psi"<<','<<"matching"<<'\n';   
    fout <<  isochrone_time <<','<< psi_time <<','<<matching_time<< '\n';	
    fout.close();
}
...
int main(int argc, char*argv[]){
  ...
  //configuration
	...
	save_runtime(iso_t,psi_t,match_t,i);
