...
void save_runtime(time_t isochrone_time, time_t psi_time, time_t matching_time, int scenario_id,Driver car, Pedestrian foot, Manager m){
    ...
    string filename="/home/matchi/Desktop/ST/results007/Runtime"+std::to_string(scenario_id) +".csv";
    ofstream fout(filename);
...
RLC::DFA dfa_passenger=RLC::pt_foot_dfa();
    ...
    ofstream fout(fileName);
    fout << "pickup driver" <<',' << "pickup pedestrian" <<',' << "dropoff driver" <<',' << "dropoff pedestrian" <<',' << "pickup set" <<','<< "dropoff set"<<','<< "shared paths"<<','<<"len1"<<','<<"len2"<<','<<"len3"<<','<<"len4"<<','<<"len5"<<'\n';   
...
int main(int argc, char*argv[]){
	...
	save_position2("/home/matchi/Desktop/ST/results007/Scenario"+std::to_string(i) +".csv", *d1, *p1,*m,g);
	save_parameter("/home/matchi/Desktop/ST/results007/Parameter"+std::to_string(i) +".csv", *d1, *p1, *m, psi_result, g );
